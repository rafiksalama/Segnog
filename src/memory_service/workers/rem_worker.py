...
from ..config import get_background_dedup_threshold
...
class REMWorker:
    ...
    async def _run_cycle(self) -> None:
        ...
        if self._dragonfly:
            ...
            asyncio.create_task(self._dragonfly.record_latency("BG:rem/cycle", dur))
        ...
        for ep in episodes:
            ...
            if embedding:
                similar = await self._find_similar_consolidated(
                    group_id, embedding, threshold=get_background_dedup_threshold()
                )
                if similar:
    ...
    async def _link_duplicate_episode(self, dup_uuid: str, original_uuid: str) -> None:
        ...
        try:
            ...
            logger.warning("Failed to link duplicate episode (non-fatal): %s", e)
    ...
    async def _decay_hebbian_weights(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Hebbian decay failed: {e}")
            return
        ...
        try:
            ...
            logger.warning(f"Hebbian prune failed: {e}")
