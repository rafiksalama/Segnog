...
async def reinforce_co_activations(
    ...
    try:
        ...
        logger.warning(f"Hebbian: trigger activation failed for {trigger_uuid[:8]}: {e}")
...
async def reinforce_knowledge_activations(
    ...
    for uuid in result_uuids:
        try:
            ...
            logger.warning(f"Hebbian: knowledge activation failed for {uuid[:8]}: {e}")
...
async def get_co_activation_weights(
    ...
    try:
        ...
        logger.warning(f"Hebbian: co-activation weight fetch failed: {e}")
        return {}
