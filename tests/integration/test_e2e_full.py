...
class TestEpisodes:
    ...
    def test_search_episodes(self, client):
        ...
        top = episodes[0]
        assert (
            "sales" in top.get("content", "").lower() or "revenue" in top.get("content", "").lower()
        )
        print(
            f"  ✓ Episode search returned {len(episodes)} results, top score={top.get('score', 0):.3f}"
        )
...
class TestKnowledge:
    ...
    def test_search_knowledge_hybrid(self, client):
        ...
        top = entries[0]
        assert (
            "revenue" in top.get("content", "").lower() or "sales" in top.get("content", "").lower()
        )
...
class TestSmartOps:
    def test_reinterpret_task(self, client):
        ...
        print(
            f"  ✓ Reinterpreted task: labels={data['search_labels']}, query='{data['search_query'][:60]}...'"
        )
...
3. [raw] System initialization complete, connected to DragonflyDB... (score=0.31)"""
                ...
                "mission_data_json": json.dumps(
                    {
                        ...
                        "state": {
                            ...
                            "outputs": [
                                {
                                    "iteration": 1,
                                    "output": "Found 3 quarterly reports via web_search",
                                },
                                {
                                    "iteration": 4,
                                    "output": "Report generated at /tmp/sales_analysis.md",
                                },
                            ],
                        },
                        "plan": {
    ...
    def test_extract_artifacts(self, client):
                ...
                "mission_data_json": json.dumps(
                    {
                        ...
                        "state": {
                            ...
                            "outputs": [
                                {
                                    "iteration": 3,
                                    "output": "file_write('/tmp/sales_analysis.md', ...) → success",
                                },
                                {
                                    "iteration": 4,
                                    "output": "file_write('/tmp/sales_summary.csv', ...) → success",
                                },
                            ],
                        },
                        "plan": {
        ...
        for e in entries[:3]:
            print(
                f"    - [{e.get('artifact_type', '?')}] {e.get('name', '')} @ {e.get('path', '')}"
            )
...
class TestStartupPipeline:
    def test_startup_pipeline_full(self, client):
        ...
        print(
            f"    - inferred_state: '{data['inferred_state'][:80]}...' "
            if data["inferred_state"]
            else "    - inferred_state: (empty)"
        )
        print(
            f"    - narrative: '{data['background_narrative'][:100]}...' "
            if data["background_narrative"]
            else "    - narrative: (empty)"
        )
...
class TestRunCuration:
    def test_run_curation_full(self, client):
        ...
        mission_data = {
            ...
            "state": {
                ...
                "outputs": [
                    ...
                    {
                        "iteration": 2,
                        "output": "Found documentation for LangChain, CrewAI, AutoGen",
                    },
        ...
        print(
            f"    - reflection: '{data['reflection'][:100]}...' ({len(data['reflection'])} chars)"
        )
...
class TestEndToEndFlow:
    ...
    def test_full_lifecycle(self, client):
        ...
        print(
            f"    ✓ Startup completed: narrative={len(startup_data['background_narrative'])} chars"
        )
        ...
        for etype, edata in [
            ...
            (
                "action",
                {"tool": "python_exec", "code": "import pandas as pd; df = pd.read_csv(...)"},
            ),
        ...
        # Update tool stats
        for tool, success, ms in [
            ("file_read", True, 50),
            ("python_exec", True, 2500),
            ("file_write", True, 80),
        ]:
            client.post(
                "/state/tool-stats",
        ...
        print(
            f"    ✓ Curation: reflection={len(curation_data['reflection'])} chars, "
